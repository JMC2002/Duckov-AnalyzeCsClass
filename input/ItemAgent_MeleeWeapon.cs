...
public class ItemAgent_MeleeWeapon : DuckovItemAgent
{
	...
	private int UpdateColliders()
	{
		...
		return Physics.OverlapSphereNonAlloc(base.Holder.transform.position, AttackRange + 0.05f, colliders, GameplayDataSettings.Layers.damageReceiverLayerMask);
	}
	...
	private int CheckCollidersInRange(bool dealDamage)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			vector.Normalize();
			if (!(Vector3.Angle(vector, base.Holder.CurrentAimDirection) < 90f) && !(magnitude < 0.4f))
			{
				continue;
			}
			num2++;
			if (dealDamage)
			{
				...
				if ((bool)setting)
				{
					...
					damageInfo.elementFactors.Add(new ElementFactor(setting.element, 1f));
					damageInfo.buff = setting.buff;
					damageInfo.buffChance = setting.buffChance;
				}
